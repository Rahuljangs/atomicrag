    ...
    def load(self) -> KnowledgeGraph:
        ...
        try:
            ...
            kus = [
                ...
                for r in session.execute(
                    self._sql_text(
                        "SELECT id, content, embedding, chunk_id, entity_ids, metadata FROM knowledge_units"
                    )
                ).fetchall()
            ]
            ...
            entities = [
                ...
                for r in session.execute(
                    self._sql_text(
                        "SELECT id, name, entity_type, embedding, metadata FROM entities"
                    )
                ).fetchall()
            ]
